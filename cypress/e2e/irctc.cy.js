...
describe("IRCTC TATKAL BOOKING", () => {
	it("Tatkal Booking Begins......", () => {
		...
		if (TATKAL && PREMIUM_TATKAL) {
			...
			).to.be.true; // Ensure at least one variable is true (or both are false)
		}
		...
		cy.submitCaptcha().then(() => {
			...
			cy.get(".ui-autocomplete > .ng-tns-c57-8")
				// .invoke("val", SOURCE_STATION)
				// .trigger("input");
				.should("be.visible")
				.type(SOURCE_STATION, { delay: 100 });
			// cy.wait(600)
			cy.get("#p-highlighted-option").should("be.visible").click();

			// to station
			cy.get(".ui-autocomplete > .ng-tns-c57-9")
				// .invoke("val", DESTINATION_STATION)
				// .trigger("input");
				.should("be.visible")
				.type(DESTINATION_STATION, { delay: 100 });
			// cy.wait(600)
			cy.get("#p-highlighted-option").should("be.visible").click();
			...
			cy.get(":nth-child(n) > .bull-back").each((div, index) => {
				...
				if (
					...
					console.log("Found Train Div Block:", div);
					cy.log("Found Train Div Block:", div);
					...
					cy.get(".ui-autocomplete input").each((inputField, index) => {
						...
						if (PASSENGER_DETAILS && index < PASSENGER_DETAILS.length) {
							...
							if (PASSENGER && PASSENGER["NAME"]) {
								...
								cy.task(
									"log",
									`'NAME' property missing in passenger object at index ${index}`,
								);
							}
						} else {
							...
							cy.task(
								"log",
								`PASSENGER_DETAILS is missing or insufficient data for index ${index}`,
							);
						}
					});
