					...
					if (MANUAL_CAPTCHA) {
						...
						cy.get(".captcha-img")
							...
							.then((value) => {
								...
								cy.exec(`python irctc-captcha-solver/app.py "${value}"`).then(
									(result) => {
										cy.get("#captcha")
											.clear()
											.type(result.stdout)
...
function solveCaptcha() {
	...
	cy.get("body")
		...
		.then((el) => {
			if (
				el[0].innerText.includes("Unable to process current transaction") &&
								...
								.each((div, index) => {
									...
									if (
										...
										cy.get(
											...
											.click();
										cy.get(
											`:nth-child(${index + 2}) > .bull-back > app-train-avl-enq > [style="padding-top: 10px; padding-bottom: 20px;"]`,
										)
											.contains("Book Now")
											.click();
